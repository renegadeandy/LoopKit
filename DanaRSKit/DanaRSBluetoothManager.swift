//
//  DanaRSBluetoothManager.swift
//  LoopKit
//
//  Created by Andy Armstrong on 10/05/2021.
...
class BluetoothManager: NSObject {
    ...
    private let managerQueue = DispatchQueue(label: "com.renegade.DanaRSKit.danaRSBluetoothManagerQueue", qos: .unspecified)

    override init() {
        super.init()

        managerQueue.sync {
            self.manager = CBCentralManager(delegate: self, queue: managerQueue, options: [CBCentralManagerOptionRestoreIdentifierKey: "com.renegade.DanaRSKit"])
        }
    }
